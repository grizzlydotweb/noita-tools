...
import { copyImage, imageToBase64 } from "../../../services/imageActions/webImageActions";
...
const MapComponent = (props: IMapProps) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      for (const y of yr) {
        for (const x of xr) {
          const mapData = infoProvider.providers.map.provide(x, y, infoProvider.config.seed);
          if (!mapData) {
            continue;
          }
          // console.log(mapData);
